...
<div class="space-y-6">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('accountant.payroll-periods.create') }}"
                   class="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-amber-600 text-white font-medium shadow-lg shadow-amber-500/20 hover:bg-amber-700 transition text-sm">
                    <span>+</span>
                    Thêm kỳ lương
                </a>
            </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-4">
            ...
            <div class="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
                ...
                <h3 class="text-2xl font-bold mt-1 text-amber-700">{{ $stats['approved'] }}</h3>
            </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-100 flex flex-wrap items-center justify-between gap-4">
                ...
                <form action="{{ route('accountant.payroll-periods.index') }}" method="GET" class="flex items-center gap-3">
                    <label for="year" class="text-sm font-semibold text-slate-600">Lọc theo năm</label>
                    <select id="year" name="year"
                            onchange="this.form.submit()"
                            class="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-800 outline-none transition focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20">
                                    ...
                                    @if ($period)
                                        ...
                                        @elseif ($period->status === 'approved')
                                            @if ($period->is_all_approved)
                                                <span class="inline-flex px-3 py-1 rounded-full text-xs font-semibold bg-violet-100 text-amber-800">Đã duyệt xong</span>
                                            @else
