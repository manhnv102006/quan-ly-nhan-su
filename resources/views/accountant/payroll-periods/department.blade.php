<x-accountant-layout title="Danh sách lương {{ $department->department_name }}">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <div class="flex items-center gap-3">
                    <a href="{{ route('accountant.payroll-periods.show', $payrollPeriod) }}" 
            ...
            <div class="flex items-center gap-3">
                @if ($departmentStatus === 'open')
                    ...
                    @if ($payrollPeriod->is_active)
                        <form action="{{ route('accountant.payroll-periods.calculate', $payrollPeriod) }}" method="POST">
                            @csrf
                            <input type="hidden" name="department_id" value="{{ $department->id }}">
                            <button type="submit" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-medium shadow-lg transition text-sm">
                                ⚡ Tính lương tự động
                            </button>
                        </form>
                    @endif
                @elseif ($departmentStatus === 'calculated')
                    ...
                    @if ($payrollPeriod->is_active)
                        <form action="{{ route('accountant.payroll-periods.recalculate', $payrollPeriod) }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn tính lại lương? Toàn bộ bảng lương hiện tại của kỳ này sẽ bị xóa và tính lại từ đầu.')">
                            @csrf
                            <input type="hidden" name="department_id" value="{{ $department->id }}">
                        ...
                        <form action="{{ route('accountant.payroll-periods.approve', $payrollPeriod) }}" method="POST">
                            @csrf
                            <input type="hidden" name="department_id" value="{{ $department->id }}">
                ...
                @elseif ($departmentStatus === 'approved')
                    <span class="inline-flex px-4 py-2 rounded-xl text-sm font-semibold bg-violet-100 text-amber-800 border border-amber-200">
                        🟣 Đã duyệt (Approved)
                    </span>
                    @if ($payrollPeriod->is_active)
                        <form action="{{ route('accountant.payroll-periods.pay', $payrollPeriod) }}" method="POST"
                ...
                @elseif ($departmentStatus === 'paid')
                    ...
                    @if ($payrollPeriod->is_active)
                        <form action="{{ route('accountant.payroll-periods.close', $payrollPeriod) }}" method="POST">
                            @csrf
                            <input type="hidden" name="department_id" value="{{ $department->id }}">
                                    ...
                                    <div class="flex justify-center items-center gap-2">
                                        <button type="button"
                                                onclick="openPayrollModal({{ json_encode([
                                                    ...
                                                    'pdf_url' => route('accountant.payrolls.pdf', $payroll),
                                                ]) }})"
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div class="relative w-full max-w-4xl transform rounded-3xl bg-white p-6 shadow-2xl transition-all border border-slate-100">
                ...
                <div class="flex items-center justify-between border-b border-slate-100 pb-4 mb-4">
                    <div>
                        <h3 class="text-xl font-bold text-slate-800 flex items-center gap-2">
                            ...
                            <span id="modalPayrollCode" class="text-amber-700 bg-amber-50 px-2 py-0.5 rounded-lg text-sm font-semibold">PL000000</span>
                        </h3>
                    </div>
                ...
                <div class="flex border-b border-slate-100 mb-6">
                    <button onclick="switchTab('payment')" id="tab-payment" class="px-4 py-2 border-b-2 border-violet-600 text-amber-700 font-semibold text-sm transition">
                        Thanh toán
                    </button>
                ...
                <div id="content-payment" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="bg-slate-50/50 rounded-2xl p-5 border border-slate-100 space-y-3.5 text-sm">
                        ...
                        <div class="border-t border-slate-200/60 my-2"></div>
                        <div class="flex justify-between items-center">
                            <span class="text-amber-800 font-bold text-base">Tổng lương (Thực lĩnh):</span>
                            <span class="font-black text-amber-800 text-lg" id="modalTotalSalary">11,050,000 ₫</span>
                        </div>
                        <div class="border-t border-slate-200/60 my-2"></div>
                        <div class="flex justify-between items-center">
                ...
                <div id="content-attendance" class="hidden space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div class="bg-amber-50/50 rounded-2xl p-4 border border-violet-100 text-center">
                            <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Ngày công chuẩn</p>
                            <h4 class="text-2xl font-black text-amber-800 mt-1" id="modalStandardDaysAtt">26</h4>
                        </div>
    ...
    <script>
        ...
        function switchTab(tab) {
            ...
            if (tab === 'payment') {
                btnPayment.className = "px-4 py-2 border-b-2 border-violet-600 text-amber-700 font-semibold text-sm transition";
                ...
                btnAttendance.className = "px-4 py-2 border-b-2 border-violet-600 text-amber-700 font-semibold text-sm transition";
        ...
        function openAdjustModal(payrollId, bonus, deduction, employeeName) {
            ...
            form.action = `{{ url('accountant/payroll-periods/'.$payrollPeriod->id.'/payrolls') }}/${payrollId}/adjust`;
