<x-accountant-layout title="Thêm kỳ lương mới">

    <div class="space-y-6">

        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-slate-800">Thêm kỳ lương mới</h2>
                <p class="text-sm text-slate-500 mt-1">Điền thông tin kỳ lương mới bên dưới</p>
            </div>

            <a href="{{ route('accountant.payroll-periods.index', ['year' => $prefillYear ?? date('Y')]) }}"
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 sm:p-8 max-w-3xl">
            ...
            <form action="{{ route('accountant.payroll-periods.store') }}" method="POST" class="space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select id="month" name="month" required
                                class="w-full rounded-xl border border-slate-200 px-4 py-3 text-slate-800 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition @error('month') border-red-400 @enderror">
                    ...
                    <div>
                        ...
                        <select id="year" name="year" required
                                class="w-full rounded-xl border border-slate-200 px-4 py-3 text-slate-800 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition @error('year') border-red-400 @enderror">
                            @php
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name"
                           ...
                           class="w-full rounded-xl border border-slate-200 px-4 py-3 text-slate-800 placeholder:text-slate-400 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition @error('name') border-red-400 @enderror">
                    @error('name')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="date" id="start_date" name="start_date"
                               ...
                               class="w-full rounded-xl border border-slate-200 px-4 py-3 text-slate-800 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition @error('start_date') border-red-400 @enderror">
                        @error('start_date')
                            <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" id="end_date" name="end_date"
                               ...
                               class="w-full rounded-xl border border-slate-200 px-4 py-3 text-slate-800 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition @error('end_date') border-red-400 @enderror">
                        @error('end_date')
                            <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function autofillDates() {
                ...
                if (month && year) {
                    ...
                    // 1. Tự động điền Tên kỳ lương (nếu user chưa sửa tay)
                    if (!nameInput.dataset.manual) {
                        nameInput.value = `Kỳ lương tháng ${formattedMonth}/${year}`;
                    }

                    // 2. Tự động tính Ngày bắt đầu (01 của tháng đó)
                    startDateInput.value = `${year}-${formattedMonth}-01`;

                    // 3. Tự động tính Ngày kết thúc (ngày cuối của tháng đó)
                    const lastDay = new Date(year, month, 0).getDate();
                    endDateInput.value = `${year}-${formattedMonth}-${lastDay.toString().padStart(2, '0')}`;
                }
            }

            // Gọi lần đầu để tự động điền dựa trên giá trị mặc định
            autofillDates();
            ...
            // Đánh dấu nếu người dùng tự ý đổi tên kì lương
