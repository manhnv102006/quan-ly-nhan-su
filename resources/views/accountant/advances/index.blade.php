...
<x-accountant-layout title="Tạm ứng lương" subtitle="Duyệt yêu cầu ứng lương">
    <div class="accountant-page">
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-slate-900">Danh sách yêu cầu tạm ứng</h2>
                <p class="text-sm text-slate-500">Duyệt, từ chối và theo dõi trạng thái hoàn trả</p>
            </div>
        ...
        <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
            ...
            @include('accountant.partials.stat-card', ['label' => 'Đã duyệt', 'value' => $stats['approved'], 'tone' => 'text-sky-600'])
            @include('accountant.partials.stat-card', ['label' => 'Đang trừ dần', 'value' => $stats['partial'], 'tone' => 'text-orange-600'])
        ...
        <form method="GET" class="accountant-card flex flex-wrap items-end gap-4 p-5">
            <div class="min-w-[180px] flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Mã, tên NV..." class="accountant-field">
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($advances as $advance)
                            <tr class="hover:bg-cyan-50/30">
                                ...
                                <td class="px-4 py-3">
                                    <p class="font-semibold">{{ $advance->employee?->full_name }}</p>
                                    <p class="text-xs text-slate-500">{{ $advance->employee?->employee_code }}</p>
                                </td>
                        ...
                        @empty
                            <tr><td colspan="7" class="px-4 py-12 text-center text-slate-500">Chưa có yêu cầu tạm ứng.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
