...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans text-gray-900 antialiased">
        @php
            ...
            $isDark = filter_var($attributes->get('dark', false), FILTER_VALIDATE_BOOLEAN);
            ...
            <div class="min-h-screen flex flex-col sm:justify-center items-center px-4 py-10 bg-gradient-to-br from-indigo-600 via-blue-600 to-cyan-500">
                <div class="text-center mb-6">
                    <a href="/" class="inline-flex flex-col items-center group">
                        ...
                        <span class="mt-3 text-xl font-bold text-white tracking-tight">{{ config('app.name', 'Quan ly nhan su') }}</span>
                    </a>
                </div>

                <div class="w-full {{ $isWide ? 'max-w-6xl' : 'sm:max-w-md' }}">
                    <div class="{{ $isFlush ? '' : 'bg-white/95 backdrop-blur-sm shadow-2xl rounded-2xl overflow-hidden border border-white/20' }}">
                        <div class="{{ $isFlush ? '' : ($isWide ? 'p-4 sm:p-6' : 'px-8 pt-8 pb-2') }}">
                            {{ $slot }}
                        </div>
                    </div>
                </div>

                <p class="mt-8 text-sm text-white/70">&copy; {{ date('Y') }} {{ config('app.name', 'Quan ly nhan su') }}</p>
            </div>
        @endif
    </body>
</html>
