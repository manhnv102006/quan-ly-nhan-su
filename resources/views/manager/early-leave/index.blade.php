<x-manager-layout
    ...
    subtitle="Duyệt đơn xin về sớm của nhân viên cấp dưới."
>
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if ($req->isPending())
                                        <div class="flex items-center justify-center gap-2">
                                            <form method="POST" action="{{ route('manager.early-leave.approve', $req) }}">
                                                @csrf @method('PATCH')
                                                <button type="submit"
                                        ...
                                        <div id="reject-modal-{{ $req->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                                            <div class="bg-white rounded-2xl p-6 shadow-xl w-full max-w-sm mx-4">
                                                ...
                                                <form method="POST" action="{{ route('manager.early-leave.reject', $req) }}">
                                                    @csrf @method('PATCH')
