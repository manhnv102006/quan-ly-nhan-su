        ...
        <div>
            @if ($hasChildren)
                <button
                    ...
                    class="manager-menu-item group w-full text-left {{ $groupHighlighted ? 'manager-menu-item-active' : 'manager-menu-item-inactive' }}"
                >
                    <span class="relative flex h-9 w-9 shrink-0 items-center justify-center rounded-xl {{ $groupHighlighted ? 'bg-white/20 text-white' : 'bg-white text-slate-500 shadow-sm shadow-slate-200/60' }}">
                        ...
                        @if (! empty($item['badge']) && $item['badge'] > 0)
                            <x-nav-badge :count="$item['badge']" :active="$groupHighlighted" dot-only active-ring="ring-teal-500" />
                        @endif
                    </span>
                    <span class="flex min-w-0 flex-1 items-center justify-between gap-2">
                        ...
                        <span class="flex shrink-0 items-center gap-2">
                            @if (! empty($item['badge']) && $item['badge'] > 0)
                                <x-nav-badge :count="$item['badge']" :active="$groupHighlighted" active-ring="ring-teal-500" />
                            @endif
                            <svg
