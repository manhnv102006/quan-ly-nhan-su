...
<header class="manager-header sticky top-0 z-50">
    <div class="flex h-[60px] items-center justify-between gap-3 px-4 lg:px-6">
        <div class="flex min-w-0 flex-1 items-center gap-3">
            <button @click="sidebarOpen = !sidebarOpen" class="rounded-lg p-2 text-slate-500 hover:bg-teal-50 lg:hidden" aria-label="Mở menu">
        ...
        <div class="flex shrink-0 items-center gap-2">
            @if (($managerPendingApprovals['total'] ?? 0) > 0)
                <a href="{{ route('manager.dashboard') }}#approvals" class="relative rounded-xl border border-slate-200 bg-white p-2 text-slate-500 shadow-sm hover:border-teal-200 hover:text-teal-700">
                    <span class="absolute -right-0.5 -top-0.5 flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-rose-500 px-1 text-[10px] font-bold text-white">{{ $managerPendingApprovals['total'] > 9 ? '9+' : $managerPendingApprovals['total'] }}</span>
            ...
            <a href="{{ route('profile.edit') }}" class="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-teal-500 to-emerald-600 text-xs font-bold text-white shadow-sm">{{ $initial }}</a>
