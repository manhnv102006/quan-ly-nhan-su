        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">
                    ...
                    <span class="font-semibold text-emerald-600">{{ $managedDepartment->department_name }}</span>
                </p>
            </div>
            ...
            <form action="{{ route('manager.notifications.store') }}" method="POST"
                ...
                <div class="space-y-2">
                    ...
                    <input id="title" name="title" type="text" value="{{ old('title') }}" required
                           ...
                           class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 focus:border-emerald-400 focus:outline-none">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea id="content" name="content" rows="5" required
                              ...
                              class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 focus:border-emerald-400 focus:outline-none">{{ old('content') }}</textarea>
                </div>
                    ...
                    <div x-show="audience === 'selected'" x-cloak class="space-y-3">
                        ...
                        <div class="max-h-72 overflow-y-auto rounded-2xl border border-slate-200 bg-white divide-y divide-slate-100">
                            @forelse ($members as $member)
                                <label class="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-emerald-50/50 transition">
                                    <input type="checkbox"
                                           name="user_ids[]"
                                           value="{{ $member->id }}"
