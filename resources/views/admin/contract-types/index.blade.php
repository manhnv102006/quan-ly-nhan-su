<x-admin-layout title="Quản lý loại hợp đồng">
    <div class="space-y-6">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-slate-800">Danh sách loại hợp đồng</h2>
                <p class="text-sm text-slate-500 mt-1">Quản lý các loại hợp đồng và thời hạn.</p>
            </div>
            <div class="flex flex-wrap items-center gap-3">
                <a href="{{ route('admin.contract-types.create') }}"
                ...
                <a href="{{ route('admin.contract-types.trash') }}"
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-100">
                <form method="GET" action="{{ route('admin.contract-types.index') }}" class="flex flex-wrap gap-3 items-center">
                    <div class="min-w-0 flex-1">
                        ...
                        <input id="search" name="search" type="text" value="{{ request('search') }}"
                               class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 focus:border-violet-400 focus:ring-violet-100 focus:outline-none" placeholder="Tìm loại hợp đồng...">
                    </div>
                    <div>
                    ...
                    <tbody>
                        @forelse($contractTypes as $type)
                            <tr class="border-t border-slate-100 hover:bg-slate-50 transition">
                                ...
                                <td class="px-6 py-4 text-slate-500">{{ $type->created_at?->format('d/m/Y') }}</td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('admin.contract-types.edit', $type) }}"
                                           class="w-9 h-9 rounded-lg bg-amber-100 text-amber-600 flex items-center justify-center hover:bg-amber-200"
                                           title="Sửa">✏️</a>
                                        <form action="{{ route('admin.contract-types.destroy', $type) }}" method="POST" id="delete-form-{{ $type->id }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" onclick="openDeleteModal('{{ $type->id }}', @json($type->contract_name))"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-12 text-slate-400">Chưa có loại hợp đồng nào.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-3xl shadow-xl w-full max-w-sm mx-4 p-6 text-center">
            ...
            <h3 class="mt-5 text-lg font-bold text-slate-800">Xóa loại hợp đồng?</h3>
            <p class="mt-2 text-sm text-slate-500">Bạn có chắc muốn xóa loại hợp đồng <span id="delete-type-name" class="font-semibold text-slate-700"></span>? Bạn có thể khôi phục nó sau.</p>
