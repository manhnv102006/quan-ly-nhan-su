    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <h2 class="mt-2 text-2xl font-bold text-slate-800">Sửa ứng viên</h2>
                <p class="text-sm text-slate-500 mt-1">Cập nhật thông tin hồ sơ ứng viên và thay CV mới nếu cần.</p>
            </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 sm:p-8 max-w-4xl">
            ...
            <form action="{{ route('admin.recruitment.candidates.update', $candidate) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                ...
                <div>
                    <label for="job_post_id" class="block text-sm font-semibold text-slate-700 mb-2">Tin tuyển dụng</label>
                    ...
                    @error('job_post_id')<p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label for="full_name" class="block text-sm font-semibold text-slate-700 mb-2">Họ và tên <span class="text-red-500">*</span></label>
                        <input type="text" id="full_name" name="full_name" value="{{ old('full_name', $candidate->full_name) }}"
                    ...
                    <div>
                        <label for="phone" class="block text-sm font-semibold text-slate-700 mb-2">Số điện thoại <span class="text-red-500">*</span></label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="email" id="email" name="email" value="{{ old('email', $candidate->email) }}"
                    ...
                    <div>
                        <label for="birth_date" class="block text-sm font-semibold text-slate-700 mb-2">Ngày sinh</label>
                ...
                <div>
                    <label for="address" class="block text-sm font-semibold text-slate-700 mb-2">Địa chỉ <span class="text-red-500">*</span></label>
                    <textarea id="address" name="address" rows="3" required placeholder="Nhập địa chỉ hiện tại"
                              class="w-full rounded-xl border border-slate-200 px-4 py-3 text-slate-800 placeholder:text-slate-400 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 outline-none transition @error('address') border-red-400 @enderror">{{ old('address', $candidate->address) }}</textarea>
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5 space-y-4">
                    <div>
                        <h3 class="text-sm font-semibold text-slate-800">CV hiện tại</h3>
                        <p class="text-sm text-slate-500 mt-1">Tải CV mới nếu bạn muốn thay thế file đang có.</p>
                    </div>

                    @if ($cvUrl)
                        <div class="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-4">
                            <p class="text-sm font-semibold text-emerald-700">CV hiện tại đang sẵn sàng</p>
                            <p class="mt-1 text-sm text-emerald-600 break-all">{{ $candidate->cv_file }}</p>
                            <a href="{{ $cvUrl }}" target="_blank" rel="noopener"
                               class="mt-3 inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white text-emerald-700 font-medium border border-emerald-200 hover:bg-emerald-100 transition">
                                Mở CV hiện tại
                            </a>
                        </div>
                    ...
                    @else
                        <div class="rounded-2xl border border-slate-200 bg-white px-4 py-4">
                            <p class="text-sm font-semibold text-slate-700">Ứng viên chưa có CV trong hệ thống</p>
                        </div>
                    @endif

                    <div>
                        <label for="cv_file" class="block text-sm font-semibold text-slate-700 mb-2">Tải CV mới</label>
                        ...
                        @error('cv_file')<p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                </div>

                <div>
                    <label for="status" class="block text-sm font-semibold text-slate-700 mb-2">Trạng thái <span class="text-red-500">*</span></label>
                    <select id="status" name="status" required
                        ...
                        <option value="new" @selected(old('status', $candidate->status) === 'new')>Mới</option>
                        <option value="interview" @selected(old('status', $candidate->status) === 'interview')>Phỏng vấn</option>
                        <option value="passed" @selected(old('status', $candidate->status) === 'passed')>Đạt</option>
                        <option value="failed" @selected(old('status', $candidate->status) === 'failed')>Không đạt</option>
                    </select>
