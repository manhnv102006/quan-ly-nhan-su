        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <div class="px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-700">
                                    {{ $candidate->birth_date?->format('d/m/Y') ?? 'Chưa cập nhật' }}
                                </div>
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div class="px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 min-h-14">
                                    {{ $candidate->address }}
                                </div>
                            </div>
                        ...
                        @if ($candidate->jobPost)
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div>
                                    ...
                                    <div class="px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-700">
                                        {{ $candidate->jobPost->department?->department_name ?? 'Chưa xác định phòng ban' }}
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-700">
                                        {{ $candidate->jobPost->quantity ?? '-' }}
                                    </div>
                                </div>
                            </div>
                        @else
                            <div class="py-10 text-center text-slate-500">
                                Ứng viên này chưa được gắn với tin tuyển dụng nào.
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="space-y-6">
                @php
                    ...
                    $initial = $parts->isNotEmpty()
                        ? \Illuminate\Support\Str::upper(\Illuminate\Support\Str::substr((string) $parts->last(), 0, 1))
                        : 'UV';
                @endphp

                <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-8 text-center">
                    <div class="w-24 h-24 mx-auto rounded-3xl bg-cyan-100 flex items-center justify-center text-3xl font-bold text-cyan-700">
                        {{ $initial }}
                    </div>
