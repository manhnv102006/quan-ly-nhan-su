    ...
    <script>
        (function () {
            ...
            form.addEventListener('submit', function (event) {
                ...
                if (!roleId.value) { setFieldError(roleId, 'Vui lòng chọn vai trò.'); valid = false; }

                const usernameValue = username.value.trim();
                if (!usernameValue) { setFieldError(username, 'Vui lòng nhập tên đăng nhập.'); valid = false; }
                else if (usernameValue.length < 3 || usernameValue.length > 50) { setFieldError(username, 'Tên đăng nhập phải từ 3 đến 50 ký tự.'); valid = false; }
                else if (!usernamePattern.test(usernameValue)) { setFieldError(username, 'Tên đăng nhập chỉ được chứa chữ cái, số, gạch ngang và gạch dưới.'); valid = false; }

                const nameValue = name.value.trim();
                if (!nameValue) { setFieldError(name, 'Vui lòng nhập họ và tên.'); valid = false; }
                else if (nameValue.length < 2) { setFieldError(name, 'Họ và tên phải có ít nhất 2 ký tự.'); valid = false; }

                const emailValue = email.value.trim();
                if (!emailValue) { setFieldError(email, 'Vui lòng nhập email.'); valid = false; }
                else if (!emailPattern.test(emailValue)) { setFieldError(email, 'Email không đúng định dạng.'); valid = false; }

                if (!status.value) { setFieldError(status, 'Vui lòng chọn trạng thái.'); valid = false; }

                if (!password.value) { setFieldError(password, 'Vui lòng nhập mật khẩu.'); valid = false; }
                else if (password.value.length < 8) { setFieldError(password, 'Mật khẩu phải có ít nhất 8 ký tự.'); valid = false; }

                if (!passwordConfirmation.value) { setFieldError(passwordConfirmation, 'Vui lòng xác nhận mật khẩu.'); valid = false; }
