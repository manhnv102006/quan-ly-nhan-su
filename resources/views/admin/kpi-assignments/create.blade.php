    ...
    <div class="space-y-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">Chọn KPI và manager — mục tiêu, thời gian lấy tự động từ KPI</p>
            </div>
        </div>
        ...
        <div class="overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm">
            ...
            <form action="{{ route('admin.kpi-assignments.store') }}" method="POST" class="space-y-6 p-8">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <select id="kpi_id" name="kpi_id" required class="w-full rounded-xl border border-slate-300 px-4 py-3 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/30">
                            ...
                            @foreach ($kpis as $kpi)
                                <option value="{{ $kpi->id }}"
                                    ...
                                    data-tasks="{{ $kpi->tasks->pluck('title')->join('||') }}"
                                    @selected(old('kpi_id') == $kpi->id)>
                                    {{ $kpi->code }} - {{ $kpi->title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <p id="manager_hint" class="mt-2 hidden text-sm text-slate-500">Vui lòng chọn KPI trước để lọc manager phù hợp.</p>
                ...
                <div>
                    ...
                    <textarea id="note" name="note" rows="4" placeholder="Ghi chú thêm khi giao KPI (tuỳ chọn)..."
                        class="w-full rounded-xl border border-slate-300 px-4 py-3 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/30">{{ old('note') }}</textarea>
